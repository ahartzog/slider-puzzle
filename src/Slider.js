...
export default class Slider extends React.Component {
  ...
  onClick(number) {
    ...
    if (this.swapIsAllowed(numberIndex, zeroIndex)) {
      ...
      if (this.checkVictory()) {
        ...
        var event = new Event("victory");
        setTimeout(() => window.dispatchEvent(event), this.animationDuration());
        setTimeout(
          () => console.log("Victory event fired!"),
  ...
  checkVictory() {
    ...
    return (
      this.arraysEqual(this.state.grid, [0, 1, 2, 3, 4, 5, 6, 7, 8]) ||
